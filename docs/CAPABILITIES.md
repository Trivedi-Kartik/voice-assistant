...
| "Add Photoshop as an app I can open" | **Always asks you out loud to confirm first.** Say "yes" — it first checks your own installed apps list (works correctly for Microsoft Store apps too), and only opens a file picker for you to browse and select manually if it can't find a clear match. Once added, say "open Photoshop" / "close Photoshop" just like any built-in app — though a Store app can only be opened this way, not closed (Windows doesn't give it a normal process name to close by). Only added on this device, for you — manage or remove what you've added in Settings → "My apps." |
