...
// control_media (a Windows PowerShell/user32.dll script) is still
// Windows-only — see docs/ARCHITECTURE.md "Linux compatibility" for what's
// covered and what's deferred. Excluding it here means the server simply
// never offers that tool schema to a non-Windows client's LLM context at
// all, rather than the model trying to call something that doesn't exist on
// this device.
const WINDOWS_ONLY_CAPABILITIES = ["control_media"];
